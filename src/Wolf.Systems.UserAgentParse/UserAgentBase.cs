...
using Wolf.Systems.UserAgentParse.Internal;
...
namespace Wolf.Systems.UserAgentParse
{
    ...
    {
        internal readonly RegexConfigurationsDefault _regexConfigurations;
            ...
            activeXObject)
        {
            ...
            CheckUserAgent(userAgent, "Android", mc2 =>
            {
                ...
                if (!string.IsNullOrEmpty(Device.Name))
                {
                    ...
                    if (androidList != null && androidList.Length > 1)
                    {
                        ...
                        {
                            var deviceType = GloableConfigurations.DeviceTypeList.Where(x => x.Value == androidList[2])
            ...
            */
                        ...
                        if (Os.Name == "Android")
                        {
                            if (GloableConfigurations.AndroidModels.Any(x => x.Key == model))
                            {
                                ...
                                if (deviceModel.Length > 0)
                                {
                                    ...
                                    {
                                        var key=GloableConfigurations.DeviceTypeList.Where(x=>x.Value==deviceModel[2]).Select(x=>x.Key).FirstOrDefault();
                                        Device.DeviceType = (DeviceType)key;
                                    }
                                }
            ...
            if (CheckUserAgent(userAgent, @"Opera", RegexOptions.IgnoreCase))
            {
                ...
                if (Browser.Name == "Opera" && Device.DeviceType == DeviceType.Mobile)
                {
            ...
            if (CheckUserAgent(userAgent, "Espial"))
            {
                ...
                if (Device.DeviceType != DeviceType.Television)
                {
                    Device.DeviceType = DeviceType.Television;
                    Device.Name = null;
                    Device.Manufacturer = null;
                ...
                CheckUserAgent(userAgent,
                    ...
                    mc2 =>
                    {
                        ...
                        if (androidInfo != null && androidInfo.Length > 0)
                        {
                            ...
                            {
                                var deviceType = GloableConfigurations.DeviceTypeList
                                    .Where(x => x.Value == androidInfo[2]).Select(x => x.Key).FirstOrDefault();
                                Device.DeviceType = (DeviceType)deviceType;
                            }
            ...
            if (Device.DeviceType == DeviceType.Television && Browser.Name == "Opera")
            {
            ...
            if (DetectCamouflage)
            {
                CheckUserAgent(userAgent, @"Mac OS X 10_6_3; ([^;]+); [a-z]{2}-(?:[a-z]{2})?\)", mc2 =>
                {
                    ...
                    if (androidInfo != null && androidInfo.Length > 0)
                    {
                        ...
                        {
                            var deviceType = GloableConfigurations.DeviceTypeList.Where(x => x.Value == androidInfo[2])
                                                        .Select(x => x.Key).FirstOrDefault();
                            Device.DeviceType = (DeviceType)deviceType;
                            Device.Identified = true;
                        }
        ...
        /// 是否伪装
        /// </summary>
        public virtual bool CamouFlage { get; set; }
        ...
        /// 是否使用特性
        /// </summary>
        public virtual bool UseFeatures { get; private set; }

        /// <summary>
        /// 是否支持ActiveX
        /// </summary>
        public virtual bool IsSupportActiveXObject { get; private set; }

        /// <summary>
        /// 是否Opera浏览器
        /// </summary>
        public virtual bool IsOpera { get; internal set; }
        ...
        /// </summary>
        /// <returns></returns>
        public virtual bool IsMozilla { get; internal set; }

        /// <summary>
        /// If it claims not to be Webkit, but it is probably Webkit running camouflage mode
        /// </summary>
        /// <returns></returns>
        public virtual bool IsWebKit { get; internal set; }

        /// <summary>
        /// 设备宽度
        /// </summary>
        public virtual int DeviceWidth { get; internal set; }

        /// <summary>
        /// 设备高度
        /// </summary>
        public virtual int DeviceHeight { get; internal set; }
    }
}
